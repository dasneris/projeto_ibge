...
|---|---|---|
| Densidade | aritmética entre colunas + ordenação | `classificacao` (Alta/Média/Baixa densidade) |
| Regiões | agregação + agrupamento | `classificacao` (nível de desenvolvimento por IDH) |
| Frota | subquery  | `categoria_frota` (acima / bem acima / muito acima) |
| Vulnerabilidade | `AND` + `OR` + `NOT` | `nivel_vulnerabilidade` (crítica/alta/moderada) |
